...
function streamFile(filepath) {
    return new Promise((resolve, reject) => {
        let fileStream = fs.createReadStream(filepath); // unlinke fs.readFile() this is an eventEmitter.
        
        fileStream.on('open', () => { // Bind open event with fileStream.
            resolve(fileStream);
        });
...
function webserver(req, res) {
    ...
    fileAccess(filepath) // filepath returned in the resolve of function fileAccess is passed down to fileReader.
        ...
        .then(content => {
            ...
            // res.end(content, 'utf-8'); Not needed because a stream is a continuous flow of data.
            fileStream.pipe(res);
        })
        .catch(error => {
            console.log(error);
